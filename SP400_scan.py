...
def get_tickers_metadata(url):
    try:
        ...
        # Wikipedia dla S&P 400 używa tabeli, gdzie kolumny to 'Security' i 'GICS Sector'
        ...
        print(f"Błąd pobierania metadanych S&P 400: {e}")
        return {}
...
def analyze_market(metadata, lookback_window=5):
    ...
    # Pobieranie danych dla MidCapów
...
def create_sector_summary(bullish, bearish):
    ...
    return f"<div style='margin:10px 0;padding:12px;background:#fcfcfc;border:1px solid #eee;display:inline-block;'>" \
           f"<h4 style='margin:0 0 8px 0;font-size:14px;'>Podsumowanie Sektorów MidCap:</h4>" \
...
def create_table_html(signals, signal_type):
    if not signals: return "<p style='color:gray;font-size:14px;'>Brak sygnałów dla S&P 400.</p>"
    ...
    for s in signals:
        ...
        adx_style = "color: #27ae60; font-weight: bold;" if s['adx'] > 25 else "color: #e67e22; font-weight: bold;" if s['adx'] >= 15 else "color: #444;"
        vol_style = "color: #27ae60; font-weight: bold;" if s['vol_ratio'] > 1.5 else "color: #e67e22; font-weight: bold;" if s['vol_ratio'] >= 1.0 else "color: #444;"
        rsi_style = "color: #27ae60; font-weight: bold;" if 30 <= s['rsi'] <= 70 else "color: #e67e22; font-weight: bold;"
        
        dist_color = "#27ae60" if (signal_type == 'bullish' and s['dist_ma20'] > 0) or (signal_type == 'bearish' and s['dist_ma20'] < 0) else "#e74c3c"
...
def main():
    ...
    full_html = f"<html><body style='font-family:Segoe UI,Arial;color:#333;font-size:15px;'><div style='background:#8e44ad;color:white;padding:20px;text-align:center;'><h2>Raport S&P 400 MidCap - {date_str}</h2></div>"
    ...
    full_html += """<div style='font-size:13px;color:gray;padding:20px;border-top:1px solid #eee;'>
        <b>Legenda kolorów S&P 400:</b><br>
        - Skany obejmują spółki o średniej kapitalizacji (MidCap).<br>
        - <b>RSI</b>: <span style='color:#27ae60;font-weight:bold;'>Zielony (30-70)</span> neutralny, <span style='color:#e67e22;font-weight:bold;'>Pomarańczowy</span> skrajne wartości.<br>
        - <b>Dystans</b>: Procentowa odległość ceny od MA20.
    </div></body></html>"""
    
    if EMAIL_SENDER and EMAIL_RECIPIENT:
        msg = EmailMessage()
        msg['Subject'] = f"🏢 Raport MidCap S&P 400 - {date_str}"
